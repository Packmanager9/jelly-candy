...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    function setUp(canvas_pass, style = "#000000") {
        ...
        window.addEventListener('pointerdown', e => {
            ...
            let circ = new Circle(TIP_engine.x, TIP_engine.y, 20, "red")
            // circ.draw()
            if(!keysPressed[' ']){
    ...
    class SpringOP {
        constructor(body, anchor, length) {
                this.body = body
                this.anchor = anchor
                this.beam = new LineOP(body, anchor, color, 1)
                this.length = length
            this.gravity = 0
            // this.width = width
        }
        ...
        clean(){
            if (this.beam.hypotenuse() > this.length*2.5) {
    ...
    class Jelly{
        constructor(x,y, rip){
            ...
            this.y = y
            this.space = 8
            for(let t = 1;t<1370;t++){
                let dot = new Circle(this.x, this.y, 3, getRandomColor(), 0, 0, .9,1)
                dot.bigbody = new Circle(this.x, this.y, this.space, getRandomColor())
                dot.touches = []
            ...
            for(let t = 0;t<this.bodies.length;t++){
                for(let k = 0;k<this.bodies.length;k++){
                    if(t!=k){
                        if(this.bodies[t].bigbody.doesPerimeterTouch(this.bodies[k])){
                            if(!this.bodies[t].touches.includes(k) && !this.bodies[k].touches.includes(t)){
                                let linker = new SpringOP(this.bodies[t], this.bodies[k], this.space)
        ...
        draw(){
            ...
            for(let t = 0;t<this.bodies.length;t++){
                if(this.bodies[t].doesPerimeterTouch(rock)){
                    ...
                    this.bodies[t].xmom += Math.cos(angle)*dis*1.1
                    this.bodies[t].ymom += Math.sin(angle)*dis*1.1



                    // this.bodies[t].xmom += (rock.xmom)/1.1
                    // this.bodies[t].ymom += (rock.ymom)/1.1
                    rock.xmom*=.99
                    rock.ymom*=.99
                }
                // this.bodies[t].ymom +=.1
                if(!this.marked2.includes(this.bodies[t])){
    ...
    let sproinger = new Jelly(260, 200, 1)
    let rock = new Circle(10, 200, 10, "white", 5, -3, 1, 1)
